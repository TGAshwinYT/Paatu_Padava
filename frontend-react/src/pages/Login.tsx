...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Returning users navigate straight to Home
      navigate('/');
    } catch (err: any) {
  ...
  const handleGoogleAuth = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        // Successful login navigates straight to Home
        navigate('/');
      } catch (err: any) {
      ...
      <div className="w-full max-w-[420px] relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-neutral-400 mt-1">
            Welcome back! Continue listening to your music.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs uppercase font-bold tracking-wider text-neutral-400">
                  Password
                </label>
                <Link
                  to="/forgot-password"
        ...
        <p className="text-center text-sm text-neutral-400 mt-8">
          ...
          <Link to="/signup" className="text-white hover:text-brand font-bold transition-colors underline-offset-4 hover:underline">
            Sign up for Paatu Paaduva
          </Link>
        </p>
      </div>
    </div>
