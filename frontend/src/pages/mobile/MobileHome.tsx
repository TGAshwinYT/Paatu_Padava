...
const MobileHome: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const favs = JSON.parse(user?.favorite_artists || '[]');
  ...
  return (
    <div className="bg-gradient-to-b from-neutral-800 to-black min-h-full p-4 pt-12 animate-in fade-in duration-1000">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold tracking-tight">{greeting()}, {user?.username || 'TamilGamingASH'}</h1>
        <div className="flex items-center gap-4">
           <Clock size={24} />
           <Settings size={24} />
           <div className="w-8 h-8 rounded-full border-2 border-green-500 overflow-hidden shadow-lg">
      ...
      <div className="grid grid-cols-2 gap-2 mb-8">
        {recentlyPlayed.length > 0 ? recentlyPlayed.map((song) => (
          ...
          >
            <img src={song.coverUrl} className="w-14 h-14 object-cover" alt="" />
          ...
          [1,2,3,4].map(i => (
            <div key={i} className="bg-white/5 rounded-md h-14 animate-pulse" />
          ))
        )}
      </div>
          ...
          {artistMixes.map((mix) => (
            <div key={mix.id} className="w-28 shrink-0">
              <div className="relative aspect-square mb-2 group">
                 <img src={mix.imageUrl} className="w-full h-full object-cover rounded shadow-2xl" alt="" />
      ...
      <section className="mb-8">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
           {recentlyPlayed.map((song) => (
              <div key={song.id} onClick={() => playTrack(song)} className="w-36 shrink-0">
                 <img src={song.coverUrl} className="w-36 h-36 object-cover rounded-lg shadow-xl mb-2" alt="" />
